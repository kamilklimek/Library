...
- `src/index.root.jsx` is the JavaScript entry point.
...
If you are concerned about using Webpack-specific semantics, you can put all your CSS right into `src/index.css`. It would still be imported from `src/index.root.jsx`, but you could always remove that import if you later migrate to a different build tool.
...
Now you can rename `src/App.css` to `src/App.scss` and update `src/app.container.jsx` to import `src/App.scss`.
...
Import Bootstrap CSS and optionally Bootstrap theme CSS in the beginning of your `src/index.root.jsx` file:
...
Finally, import the newly created `.scss` file instead of the default Bootstrap `.css` in the beginning of your `src/index.root.jsx` file, for example:
...
4. Add `// @flow` to any files you want to type check (for example, to `src/app.container.jsx`).
...
To try it, delete all the code in `src/app.container.jsx` and replace it with any of the examples on its website. The [Basic Example](https://reacttraining.com/react-router/web/example/basic) is a good place to get started.
...
We recommend to put the test files (or `__tests__` folders) next to the code they are testing so that relative imports appear shorter. For example, if `App.test.js` and `app.container.jsx` are in the same folder, the test just needs to `import App from './App'` instead of a long relative path. Colocation also helps find tests more quickly in larger projects.
...
following in their [`src/index.root.jsx`](src/main/js/index.root.jsx) file:
...
1. Users aren't always familiar with offline-first web apps. It can be useful to
   ...
   starting point, you can make use of the logic included in [`src/serviceWorker.js`](src/main/js/serviceWorker.js), which
...
- If the watcher doesn’t see a file called `index.root.jsx` and you’re referencing it by the folder name, you [need to restart the watcher](https://github.com/facebook/create-react-app/issues/1164) due to a Webpack bug.
