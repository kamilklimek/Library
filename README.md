...
- `src/index.root.js` is the JavaScript entry point.
...
If you are concerned about using Webpack-specific semantics, you can put all your CSS right into `src/index.css`. It would still be imported from `src/index.root.js`, but you could always remove that import if you later migrate to a different build tool.
...
Now you can rename `src/App.css` to `src/App.scss` and update `src/app.container.js` to import `src/App.scss`.
...
Import Bootstrap CSS and optionally Bootstrap theme CSS in the beginning of your `src/index.root.js` file:
...
Finally, import the newly created `.scss` file instead of the default Bootstrap `.css` in the beginning of your `src/index.root.js` file, for example:
...
4. Add `// @flow` to any files you want to type check (for example, to `src/app.container.js`).
...
To try it, delete all the code in `src/app.container.js` and replace it with any of the examples on its website. The [Basic Example](https://reacttraining.com/react-router/web/example/basic) is a good place to get started.
...
We recommend to put the test files (or `__tests__` folders) next to the code they are testing so that relative imports appear shorter. For example, if `App.test.js` and `app.container.js` are in the same folder, the test just needs to `import App from './App'` instead of a long relative path. Colocation also helps find tests more quickly in larger projects.
...
following in their [`src/index.root.js`](src/main/js/index.root.js) file:
...
- If the watcher doesn’t see a file called `index.root.js` and you’re referencing it by the folder name, you [need to restart the watcher](https://github.com/facebook/create-react-app/issues/1164) due to a Webpack bug.
